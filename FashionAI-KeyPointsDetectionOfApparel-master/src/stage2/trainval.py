...
def train(data_loader, net, loss, optimizer, lr):
    ...
    for i, (data, heatmaps, vismaps) in enumerate(data_loader):
        ...
        heatmaps = heatmaps.cuda(non_blocking=False)
        vismaps = vismaps.cuda(non_blocking=False)
        ...
        optimizer.step()
        metrics.append(
            [loss_output[0].item(), loss_output[1].item(), loss_output[2].item()])
    end_time = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-r', '--resume', help='specify the checkpoint', default=None)
    ...
    lrs = LRScheduler(lr, patience=3, factor=0.1,
                      min_lr=0.01*lr, best_loss=best_val_loss)
    for epoch in range(start_epoch, epochs + 1):
        ...
        print_log(epoch, lr, train_metrics, train_time, val_metrics,
                  val_time, save_dir=save_dir, log_mode=log_mode)
