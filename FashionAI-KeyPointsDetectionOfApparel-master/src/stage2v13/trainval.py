...
def train(data_loader, net, loss, optimizer, lr):
    ...
    for i, (data, heatmaps, vismaps) in enumerate(data_loader):
        ...
        heatmaps = Variable(heatmaps.cuda(non_blocking=False))
        vismaps = Variable(vismaps.cuda(non_blocking=False))
        ...
        optimizer.step()
        metrics.append(
            [loss_output[0].data[0], loss_output[1].data[0], loss_output[2].data[0]])
...
def validate(data_loader, net, loss):
    ...
    for i, (data, heatmaps, vismaps) in enumerate(data_loader):
        ...
        heatmaps = Variable(heatmaps.cuda(non_blocking=False), volatile=True)
        vismaps = Variable(vismaps.cuda(non_blocking=False), volatile=True)
        heat_pred1, heat_pred2 = net(data)
        loss_output = loss(heatmaps, heat_pred1, heat_pred2, vismaps)
        metrics.append(
            [loss_output[0].data[0], loss_output[1].data[0], loss_output[2].data[0]])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--clothes',
                        help='one cloth type', default='outwear')
