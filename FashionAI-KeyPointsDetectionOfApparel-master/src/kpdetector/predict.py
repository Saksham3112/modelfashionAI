...
def compute_keypoints(config, img0, net, encoder, doflip=False):
    ...
    img = np.transpose(img, (2, 0, 1)).astype(
        np.float32)  # channel, height, width
    img[[0, 2]] = img[[2, 0]]
    img = img / 255.0
    img = (img - config.mu) / config.sigma
    pad_imgs = np.zeros(
        [1, 3, config.img_max_size, config.img_max_size], dtype=np.float32)
    pad_imgs[0, :, :img_h2, :img_w2] = img
    data = torch.from_numpy(pad_imgs)
    data = data.cuda(non_blocking=False)
    _, hm_pred = net(data)
    ...
    if doflip:
        ...
        a[:, :, :img_w2 //
            config.hm_stride] = np.flip(hm_pred[:, :, :img_w2 // config.hm_stride], 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--clothes',
                        help='specify the clothing type', default='outwear')
    parser.add_argument('-g', '--gpu', help='cuda device to use', default='0')
    # config.proj_path + 'checkpoints/outwear_036_lgtrain.ckpt'
    parser.add_argument(
        '-m', '--model', help='specify the model', default=None)
    parser.add_argument(
        '-v', '--visual', help='whether visualize result', default=False)
    ...
    for idx in tqdm(range(test_kpda.size())):
        ...
        with torch.no_grad():
            ...
            hm_pred_flip = compute_keypoints(
                config, img0_flip, net, encoder, doflip=True)
        x, y = encoder.decode_np(
            hm_pred+hm_pred_flip, scale, config.hm_stride, (img_w/2, img_h/2), method='maxoffset')
        keypoints = np.stack([x, y, np.ones(x.shape)], axis=1).astype(np.int16)
        ...
        for k, kpt_name in enumerate(config.keypoints[config.clothes]):
            df.at[idx, kpt_name] = str(
                keypoints[k, 0])+'_'+str(keypoints[k, 1])+'_1'

        if args.visual:
            kp_img = draw_keypoints(img0, keypoints)
            cv2.imwrite(config.proj_path +
                        '/tmp/{0}{1}.png'.format(config.clothes, idx), kp_img)

    df.fillna('-1_-1_-1', inplace=True)
    print(df.head(5))
    df.to_csv(config.proj_path+'kp_predictions/' +
              config.clothes+'.csv', index=False)
