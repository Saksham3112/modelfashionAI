...
def predict(data_loader, net, config):
    ...
    for i, (imgs, reg_targets, cls_targets) in enumerate(data_loader):
        data = Variable(imgs.cuda(non_blocking=False), volatile=True)
        ...
        for j in range(len(imgs)):
            ...
            bboxes, scores = encoder.decode(reg_pred.data, cls_pred.data.squeeze(), [
                                            config.img_max_size, config.img_max_size])
            bboxes2, scores2 = encoder.decode(
                reg_target, cls_target, [config.img_max_size, config.img_max_size])
            img = np.transpose(img.numpy(), (1, 2, 0))
            img = ((img * config.sigma + config.mu) * 255).astype(np.uint8)
            draw_bbox(img, bboxes.numpy(), scores.numpy(
            ), '/home/storage/lsy/fashion/predictions/'+config.clothes+'/%d-%d.png' % (i, j), bboxes2.numpy())
...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(
        root_path + 'checkpoints/rpn_053.ckpt')  # must before cuda
